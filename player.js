  ...
  async function loadMedia() {
    ...
    try {
      if (/abyssplayer/i.test(url)) {
        ...
        elements.empty.querySelector('p').textContent = 'Gunakan URL MP4, Abyssplayer, atau YouTube agar pemutar tetap aman dan stabil.';
        setLoading(false);
        ...
        elements.empty.classList.remove('hidden');
        elements.empty.classList.add('flex');
        elements.empty.querySelector('h2').textContent = 'Stream HLS/DASH dinonaktifkan';
        elements.empty.querySelector('p').textContent = 'Pakai MP4, Abyssplayer, atau YouTube agar pemutar tetap ringan.';
        setLoading(false);
      } else {
